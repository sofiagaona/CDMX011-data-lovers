...
const objnivel = document.getElementById("nivel");
...
document.getElementById("OrdAscDescRes").addEventListener("change", filtrar);
document.getElementById("filtroTres").addEventListener("change",filtrar);
  ...
  function addPropFiltro2() { 
    ...
    for (let i=0; i<=data.pokemon.length-1; i++){
      ...
      if(typeof data.pokemon[i][optionFiltro1] == "string"){ //name, num, huevo
        ...
        if(typeof data.pokemon[i][optionFiltro1].length == "undefined"){
          tipo = Object.keys(data.pokemon[i][optionFiltro1]); //resistencia evolucion
            for(let k=0; k<=tipo.length-1;k++){
        ...
        for(let j=0; j<=data.pokemon[i][optionFiltro1].length-1;j++){
          tipoArray="";
          if(typeof data.pokemon[i][optionFiltro1][j] == "object"){ //mov. rap y atque especial
 ...
 function filtrar() {
    const objInfoFil = filterData(data, objFiltroUno.value, objFiltroDos.value, objFilTres.value, objnivel.value);
    const objInfFilOrg= sortData(objInfoFil,objFilAlfabNum.value,objFilAscDsc.value);
    var listPok = objInfFilOrg.map(function(pok){
    
     return '<li> <div class="boxTarjeta"><div class="boxImg"><figure><a href=pokemon.html><img src='+pok.img+'></a></figure></div><div class="boxInf"><p class="namePok">'+pok.name+'  '+pok.num+'</p><p><span class="texto">Tipo:</span>'+traductor(pok.type)+' </p><p><span class="texto">Fuerte Contra:</span> '+traductor(pok.resistant)+'</p> <p> <span class="texto">Debil Contra:</span> '+traductor(pok.weaknesses)+'</p><p><span class="texto">Ataque Base:</span> '+pok.stats['base-attack']+'</p><p> <span class="texto">Defensa Base:</span> '+pok.stats['base-defense']+'</p></div></div></li>' 
    })
